...
function FooterComponent() {
    ...
    return (
        <div className="md:space-y-12 fixed bottom-0 md:h-screen md:w-1/4 rounded-t-3xl md:rounded-t-none md:rounded-r-3xl p-10 text-2xl md:text-2xl justify-between md:justify-center items-center md:items-start flex md:flex-col flex-row w-full bg-gray-900 h-16 text-white">
            <div onClick={() => router.push(`/${uid}`)} class={router.pathname == `/[uid]` ? "md:flex justify-start md:bg-gray-600 md:py-7 md:pl-6 md:pr-40 md:rounded-xl md:text-purple-500" : "md:flex md:justify-start md:py-7 md:pl-6 md:pr-40 md:rounded-xl"}>
                {
                    uid && <FontAwesomeIcon
                        icon={faHome}
                        size="lg"
            ...
            <div onClick={() => router.push(`/start/${uid}`)} class={router.pathname == `/start/[uid]` || router.pathname == `/start/subject/[uid]` || router.pathname == `/filter/[uid]` ? "md:flex justify-start md:bg-gray-600 md:py-7 md:pl-6 md:pr-40 md:rounded-xl md:text-purple-500" : "md:flex md:justify-start md:py-7 md:pl-6 md:pr-40 md:rounded-xl"}>
                {
                    uid && <FontAwesomeIcon
            ...
            <div onClick={() => router.push(`/config/${uid}`)} class={router.pathname == "/config/[uid]" ? "md:flex justify-start md:bg-gray-600 md:py-7 md:pl-6 md:pr-40 md:rounded-xl md:text-purple-500" : "md:flex md:justify-start md:py-7 md:pl-6 md:pr-40 md:rounded-xl"}>
                {
                    uid && <FontAwesomeIcon 
                        icon={faUser} 
                        size="lg" 
